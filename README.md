...
```mermaid
flowchart LR
    subgraph chainguard["Chainguard Ecosystem"]
        direction TB
        subgraph wolfi["Wolfi OS (APK repository)"]
            WS["Source packages\nbuilt from source"]
            WSIG["Signed repository index\nAPKINDEX (cryptographic)"]
            WS --> WSIG
        end
        subgraph tools["Chainguard Tools"]
            MEL["melange\nBuild APKs from YAML spec\nbubblewrap isolation"]
            APK_T["apko\nAssemble OCI images\nfrom APK packages only"]
        end
    end

    subgraph project["This Project"]
        direction TB
        MY["melange.yaml\nDeclares tool binaries\n+ pinned SHA256 checksums"]
        AY["apko.yaml\nDeclares image contents\nuser · entrypoint · env"]
        KEYpr["melange.rsa\nEphemeral private key\n(never committed)"]
        KEYpu["melange.rsa.pub\nPublic key\n(committed)"]
    end

    subgraph upstream["Upstream Tool Binaries"]
        KB["kubectl"]
        KL["kubelogin"]
        TF["Terraform"]
        VD["vendir"]
    ...
    subgraph outputs["Verified Outputs"]
        APK["azdo-agent-tools.apk\nSigned APK\n(SHA256-verified contents)"]
        IDX["APKINDEX.tar.gz\nSigned index"]
        IMG["OCI image\n(APKs only — no curl, no apt)"]
        SBM["SPDX 2.3 SBOM\nauto-generated by apko"]
    end

    upstream --SHA256 verified by melange--> MY
    MY --> MEL
    KEYpr --> MEL & IDX
    KEYpu --> AY
    MEL --> APK --> IDX
    WSIG --signed Wolfi APKs\npython3 · git · jq · az · curl--> APK_T
    APK & AY --> APK_T
    APK_T --> IMG & SBM
```
...
flowchart TD
    subgraph src["Source Files"]
        MY["melange.yaml\nAPK build spec + SHA256 checksums"]
    ...
    subgraph ci["GitHub Actions Workflows"]
        ...
        PW["publish.yml\non: release published\nBuild, push & upload assets"]
    end

    subgraph build["Build Pipeline  (melange + apko)"]
        KG["melange keygen\nephemeral signing key"]
        MB["melange build\nSHA256-verified downloads → amd64 package"]
        IDX["melange index\nSigned APKINDEX.tar.gz"]
        APK["azdo-agent-tools.apk\nSigned APK"]
        WP["Wolfi base packages\npython3 · git · jq · az · openssh · curl"]
        AB["apko build\nAssemble OCI image — no Dockerfile, no shell"]
        TAR["azdo-agent-apko.tar\nOCI image tar"]
        SBOM["sbom-*.spdx.json\nSPDX 2.3 SBOM"]
    end

    subgraph publish["Published Artifacts"]
        GHCR["ghcr.io/bjrooney/azdo-agent\n:latest  ·  :vX.Y.Z"]
        GHR["GitHub Release assets\n.tar  ·  .apk  ·  APKINDEX  ·  SBOM"]
    end

    subgraph runtime["Runtime"]
        DR["docker run\nAZP_URL · AZP_TOKEN · AZP_POOL"]
        AZD["Azure DevOps\nSelf-Hosted Agent"]
    end
    ...
    BW --triggers--> KG
    RP --on merge creates release--> PW
    PW --triggers--> KG

    TAR --docker load + push--> GHCR
    TAR & SBOM & APK --> GHR

    GHCR --docker pull--> DR --> AZD
```
